                ...
                fetch(url, options)
                    ...
                    .then(responseJson => {
                        ...
                        if (responseJson.code === 16) {
                            ...
                            AsyncStorage.removeItem("jwt_token", err =>
                                console.log("removed jwt_token", err)
                            );
...
export const Products = {
        ...
        AsyncStorage.getItem("saved_products")
            .then(saved_products => {
                ...
                AsyncStorage.setItem(
                    "saved_products",
                    JSON.stringify(saved_products_array)
                );
                success(product);
            })
            .catch(err => {
    ...
    removeSavedProduct: (product, success, failure) => {
        AsyncStorage.getItem("saved_products")
            .then(saved_products => {
                if (saved_products != null) {
                    saved_products_array = JSON.parse(saved_products);
                    saved_products_array.splice(
                        saved_products_array.indexOf(product.product.prd_uuid),
                        1
                    );
                    AsyncStorage.setItem(
                        "saved_products",
    ...
    searchProducts: (searchOptions, success, failure) => {
        ...
        var searchQueryString =
            "lat=" +
            searchOptionsObj.lat +
            "&lon=" +
            searchOptionsObj.long +
            "&radius=" +
            searchOptionsObj.radius +
            "&name=" +
            searchOptionsObj.name;
        searchQueryString +=
            searchOptions.category > 0
                ? "&category=" + categories[searchOptionsObj.category].value
                : "";
    ...
    getProducts: (product_ids, success, failure) => {
        ...
        console.log(uuids);
        fetchAPI(
            "http://" + host + ":" + port + "/" + version + "/get_products",
            {
                method: "POST",
                headers: {
                    Accept: "application/json"
    ...
    updateProduct: (updatedProduct, success, failure) => {
        AsyncStorage.getItem("user_id").then(user_id => {
            ...
            productUploadRequestData.inventory.item.created_at =
                current_timestamp + "";
            productUploadRequestData.inventory.item.updated_at =
                current_timestamp + "";
            Object.keys(updatedProduct.markedDates).forEach(key => {
                if (updatedProduct.markedDates[key].selected) {
                    productUploadRequestData.inventory.item.availability.dates.push(
                        key
                    );
                }
            });
            productUploadRequestData.product.prd_info.user_uuid = user_id;
            productUploadRequestData.product.prd_info.name =
                updatedProduct.title;
            productUploadRequestData.product.prd_info.description =
                updatedProduct.about;
            productUploadRequestData.product.prd_info.category =
                updatedProduct.category;
            productUploadRequestData.product.prd_info.price =
                updatedProduct.price_daily + "";
            productUploadRequestData.product.prd_info.location.location.latitude =
                updatedProduct.productLocation.latitude + "";
            productUploadRequestData.product.prd_info.location.location.longitude =
                updatedProduct.productLocation.longitude + "";
            productUploadRequestData.product.prd_info.location.location.radius =
                defaultRadius + "";
            productUploadRequestData.product.prd_info.thumbnail =
                updatedProduct.thumbnail;
            productUploadRequestData.product.prd_info.other_images =
                updatedProduct.other_images;
...
export const User = {
    createUser: (user, success, failure) => {
        fetchAPI(
            "http://" + host + ":" + port + "/" + version + "/update_user",
            {
                method: "POST",
                headers: {
                    Accept: "application/json",
    ...
    getUser: (uuid, success, failure) => {
        ...
        fetchAPI(
            "http://" + host + ":" + port + "/" + version + "/get_user/" + uuid,
            {
                method: "GET",
                headers: {
                    Accept: "application/json"
    ...
    register: (user, success, failure) => {
        fetchAPI(
            "http://" + host + ":" + auth_port + "/" + version + "/register",
            {
                method: "POST",
                headers: {
                    Accept: "application/json",
            ...
            "http://" + host + ":" + port + "/" + version + "/order",
            {
                ...
                body: JSON.stringify({
                    items: [
                        {
                            type: "RESERVED",
                            product_id: order.productInfo.prd_uuid,
                            rentee_id: "string",
                            rate: {
                                type: "RATE_RESERVED",
                                amount: "string",
